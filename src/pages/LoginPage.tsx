        ...
        <Card className="shadow-elevated border-border/30 backdrop-blur-sm bg-card/95">
          ...
          <CardContent>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={async () => {
                      if (!email) { toast.error(t('login.enterEmailFirst')); return; }
                      const { error } = await supabase.auth.resetPasswordForEmail(email, {
                        redirectTo: `${window.location.origin}/reset-password`,
                      });
                      if (error) toast.error(error.message);
                      else toast.success(t('login.resetEmailSent'));
                    }}
            ...
            <p className="text-center text-sm text-muted-foreground mt-6">
              {t('login.noAccount')}{' '}
              <Link to="/signup" className="text-primary font-semibold hover:underline">{t('common.signup')}</Link>
            </p>
            <p className="text-center text-xs text-muted-foreground mt-2">
              <Link to="/admin-login" className="text-accent hover:underline">{t('login.adminSignIn')}</Link>
            </p>
          </CardContent>
        </Card>
      </motion.div>
