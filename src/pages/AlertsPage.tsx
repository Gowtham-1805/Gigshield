...
export default function AlertsPage() {
  ...
  const fetchAlerts = async () => {
    ...
    // 1. Fetch weather readings (last 20)
      ...
      for (const r of readings) {
        ...
        if (warnings.length > 0) {
          const severity: AlertItem['severity'] =
            (r.aqi && r.aqi > 400) || (r.rainfall && r.rainfall > 80) || (r.temperature && r.temperature > 45)
              ? 'danger'
              : 'warning';
          ...
          allAlerts.push({
            id: `weather-${r.id}`,
            type: 'weather',
            title: severity === 'danger' ? 'Severe Weather Alert' : 'Weather Warning',
            description: warnings.join(' • '),
            icon: r.rainfall && r.rainfall > 30 ? '🌧️' : r.temperature && r.temperature > 42 ? '🔥' : r.aqi && r.aqi > 300 ? '😷' : '💨',
    ...
    // 2. Fetch recent incidents as proxy for AI/trigger alerts
    ...
    if (incidents) {
      for (const inc of incidents) {
        ...
        allAlerts.push({
          id: `incident-${inc.id}`,
          type: 'prediction',
          title: `${trigger?.icon || '⚡'} ${trigger?.label || inc.trigger_type} Detected`,
          description: `Severity: ${inc.severity}/100${inc.is_simulated ? ' (Simulated)' : ''} — ${trigger?.threshold || 'Threshold exceeded'}`,
          icon: trigger?.icon || '⚡',
          timestamp: inc.created_at,
          severity: inc.severity >= 80 ? 'danger' : inc.severity >= 50 ? 'warning' : 'info',
        });
      }
    }

    // 3. Fetch claim status updates
      ...
      if (claims) {
        for (const claim of claims) {
          ...
          const statusMap: Record<string, { label: string; severity: AlertItem['severity'] }> = {
            ...
            processing: { label: 'Under Review 🔄', severity: 'warning' },
            flagged: { label: 'Flagged for Review 🚩', severity: 'danger' },
          ...
          allAlerts.push({
            id: `claim-${claim.id}`,
            type: 'claim',
            title: `Claim ${status.label}`,
            description: `₹${claim.amount} — ${trigger?.label || claim.trigger_type} • Fraud Score: ${(claim.fraud_score * 100).toFixed(0)}%`,
            icon: claim.status === 'approved' ? '✅' : claim.status === 'flagged' ? '🚩' : claim.status === 'rejected' ? '❌' : '🔄',
    ...
    <div className="min-h-screen bg-background pb-8">
      ...
      <header className="sticky top-0 z-50 glass border-b border-border/30">
        <div className="flex items-center gap-3 h-14 px-4 max-w-lg mx-auto">
          <Button variant="ghost" size="icon" onClick={() => navigate('/worker')} className="shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-display font-bold text-base tracking-tight">Alerts</h1>
              <p className="text-[10px] text-muted-foreground">{alerts.length} notifications</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 py-4 max-w-lg mx-auto space-y-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-4 w-full bg-muted/50">
            ...
            <TabsTrigger value="prediction" className="text-xs">Incidents</TabsTrigger>
            <TabsTrigger value="claim" className="text-xs">Claims</TabsTrigger>
          </TabsList>
        </Tabs>
        ...
        {!loading && filteredAlerts.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
            ...
            <p className="font-display font-semibold text-foreground">No alerts yet</p>
            <p className="text-sm text-muted-foreground mt-1">
              {activeTab === 'all' ? "You're all clear! No alerts at this time." : `No ${activeTab} alerts found.`}
            </p>
          </motion.div>
        )}
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-lg ${
                      alert.severity === 'danger' ? 'bg-destructive/10' :
                      alert.severity === 'warning' ? 'bg-accent/10' : 'bg-secondary/10'
                    }`}>
                      {alert.icon}
                    </div>
