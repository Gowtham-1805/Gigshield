...
import {
  ...
  Calendar, Loader2, RefreshCw, IndianRupee, Umbrella, Flame, Wind
} from 'lucide-react';
...
const HOURLY_RATE = 150; // ₹150/hr lost income rate
...
export default function EarningsReportPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (policies?.length) {
        ...
        const processedClaims = (claimsData || []).map((c: any) => ({
          ...c,
  ...
  // Estimated income lost (based on severity and hours — simplified model)
  const estimatedIncomeLost = approvedClaims.reduce((s, c) => {
    const severity = c.incident?.severity || 70;
    const hoursLost = Math.max(2, Math.round(severity / 15)); // 2-7 hours based on severity
    const tierMult = TIER_MULTIPLIER[policy?.tier || 'STANDARD'] || 1.0;
    return s + (hoursLost * HOURLY_RATE * tierMult);
  }, 0);

  const coverageRate = estimatedIncomeLost > 0
    ? Math.round((totalClaimAmount / estimatedIncomeLost) * 100)
    : 0;
  ...
  const pieData = Object.entries(triggerBreakdown).map(([key, val]) => ({
    name: triggerLabels[key]?.label || key,
    value: val.amount,
    icon: triggerLabels[key]?.icon || '⚠️',
  }));
  ...
  // Monthly trend (simplified — group claims by week)
  const weeklyData = (() => {
        ...
        <div className="flex items-center justify-between h-14 px-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="font-display font-bold">Earnings Impact</h1>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <ThemeToggle />
      ...
      <main className="container mx-auto px-4 py-6 max-w-4xl space-y-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-sm text-muted-foreground">Loading your earnings report...</p>
          </div>
        ) : (
          <>
            {/* Hero Value Card */}
              ...
              <Card className="shadow-card overflow-hidden relative">
                ...
                <CardContent className="relative p-6">
                  <div className="text-center mb-5">
                    <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">GigShield Protected You</p>
                    <p className="font-display text-4xl font-bold text-primary">
                      ₹{totalClaimAmount.toLocaleString('en-IN')}
                    </p>
                    <p className="text-sm text-muted-foreground mt-1">
                      against ₹{estimatedIncomeLost.toLocaleString('en-IN')} estimated income loss
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 mb-5">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className={cn(
                        'font-bold',
                        coverageRate >= 60 ? 'text-secondary' : coverageRate >= 30 ? 'text-accent' : 'text-destructive'
                      )}>
                        {coverageRate}%
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-3 rounded-lg bg-primary/10 border border-primary/20 text-center">
                      ...
                      <p className="text-[10px] text-muted-foreground">Premiums Paid</p>
                    </div>
                    <div className={cn(
                      'p-3 rounded-lg border text-center',
                      netBenefit >= 0 ? 'bg-secondary/10 border-secondary/20' : 'bg-destructive/10 border-destructive/20'
                    )}>
                      <IndianRupee className={cn('w-4 h-4 mx-auto mb-1', netBenefit >= 0 ? 'text-secondary' : 'text-destructive')} />
                      <p className={cn('font-display font-bold text-lg', netBenefit >= 0 ? 'text-secondary' : 'text-destructive')}>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              <Card className={cn(
                ...
                roi >= 0 ? 'border-l-secondary' : 'border-l-accent'
              )}>
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <div className={cn(
                      'w-10 h-10 rounded-full flex items-center justify-center shrink-0',
                      roi >= 0 ? 'bg-secondary/10' : 'bg-accent/10'
                    )}>
                      <TrendingUp className={cn('w-5 h-5', roi >= 0 ? 'text-secondary' : 'text-accent')} />
                    </div>
                    <div>
                      <p className="text-xs font-medium text-muted-foreground mb-1">Your GigShield ROI</p>
                      {approvedClaims.length > 0 ? (
                        <p className="text-sm text-foreground/80">
                          You paid <strong>₹{totalPremiumsPaid}</strong> in premiums and received <strong>₹{totalClaimAmount.toLocaleString('en-IN')}</strong> in claims.
                          {netBenefit > 0
                            ? ` That's a net gain of ₹${netBenefit.toLocaleString('en-IN')} — your coverage paid for itself ${(totalClaimAmount / Math.max(totalPremiumsPaid, 1)).toFixed(1)}x over! 🎉`
                            : ` Your coverage is keeping you protected for when disruptions hit. Stay covered! 🛡️`
                          }
                        </p>
                      ) : (
                        <p className="text-sm text-foreground/80">
                          No approved claims yet. When disruptions hit your zone, GigShield will automatically protect your income. Stay covered! 🛡️
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
                <Card className="shadow-card">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-display">Claims by Disruption Type</CardTitle>
                    <CardDescription className="text-xs">What weather events affected your income</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            {weeklyData.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                <Card className="shadow-card">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-display">Income Lost vs Covered</CardTitle>
                    <CardDescription className="text-xs">Weekly comparison of estimated loss and GigShield coverage</CardDescription>
                  </CardHeader>
                  <CardContent>
                          ...
                          content={({ active, payload, label }) => {
                            ...
                            return (
                              <div className="bg-card border border-border rounded-lg p-2 shadow-lg text-xs">
                                ...
                                <p className="text-destructive">Lost: ₹{payload[0]?.value?.toLocaleString('en-IN')}</p>
                                <p className="text-secondary">Covered: ₹{payload[1]?.value?.toLocaleString('en-IN')}</p>
                              </div>
                            );
                          }}
                        />
                        <Bar dataKey="lost" name="Income Lost" fill="hsl(var(--destructive))" radius={[4, 4, 0, 0]} opacity={0.4} />
                        <Bar dataKey="claimed" name="GigShield Covered" fill="hsl(var(--secondary))" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
            ...
            {approvedClaims.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
                <Card className="shadow-card">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-display">Claim-by-Claim Breakdown</CardTitle>
                  </CardHeader>
                    ...
                    {approvedClaims.map((claim) => {
                      ...
                      return (
                        <div key={claim.id} className="p-3 rounded-lg bg-muted/30 border border-border/50 space-y-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-muted-foreground">
                                  ...
                                  {' · '}{hoursLost}hrs lost · Severity {severity}
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="flex items-center gap-2">
                            <Progress value={Math.min(coverPct, 100)} className="h-1.5 flex-1" />
                            <span className="text-[10px] font-medium text-muted-foreground">{coverPct}%</span>
                          </div>
                        </div>
                      );
                    })}
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                <Card className="shadow-card">
                  <CardContent className="p-8 text-center">
                    ...
                    <p className="font-display font-bold text-lg mb-1">No Claims Yet</p>
                    <p className="text-sm text-muted-foreground">
                      When weather disruptions affect your zone, GigShield will automatically file claims and protect your income. Your coverage is active and monitoring.
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
