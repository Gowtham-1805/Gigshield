    ...
    <div className="space-y-4">
      ...
      <ScrollArea className="h-[240px] rounded-lg border border-border">
        <div className="p-2 space-y-3">
          {filtered.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-6">No zones found for "{search}"</p>
          )}
            ...
            <div key={city}>
              ...
              <div className="space-y-1">
                {cityZones.map(z => {
                  ...
                  const riskColor = z.risk_score > 0.7 ? 'text-destructive' :
                    z.risk_score > 0.4 ? 'text-accent' : 'text-secondary';
                  const riskLabel = z.risk_score > 0.7 ? '🔴 High' :
                    z.risk_score > 0.4 ? '🟡 Med' : '🟢 Low';

                  return (
                    <button
                      key={z.id}
                      onClick={() => setZoneId(z.id)}
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-md text-left transition-colors text-sm ${
                        isSelected
